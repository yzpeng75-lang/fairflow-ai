...
export default function App() {
  ...
  return <main className="shell" aria-busy={busy}>
    ...
    <section className="audit-card">
      ...
      {evidence.length === 0 ? <p className="empty">Capture the product page first, continue checkout, then capture again before payment.</p> : <ol className="timeline">{evidence.map((item) => <li key={item.step}><span>S{item.step}</span><div><strong>{item.pageType}</strong><small>{item.currency} {item.visibleTotal.toFixed(2)} · {item.mandatoryFees.length} fees · {item.paidChoices.length} choices · {item.renewalTerms.length} renewal terms{item.trialOfferObserved ? " · trial observed" : ""}</small></div></li>)}</ol>}
    </section>
